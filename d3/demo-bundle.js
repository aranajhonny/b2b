  ...
  return (function e(t, n, r) {
          ...
          function runDemo(options, done) {
            ...
            db.init(function(err) {
              ...
              initDB(db, function(err, root, map, wasEmpty) {
                ...
                window.ctrl = window.controller = new o.Controller(
                  model,
                  o.ctrlOptions
                );
                window.view = window.view = ctrl.setView(o.View, o.viewOptions);
                if (wasEmpty) {
          ...
          function initDB(db, done) {
            db.findAll("root", function(err, roots) {
              ...
              db.findAll("node", function(err, nodes) {
                ...
                if (!nodes.length)
                  return done(
                    new Error("Data corrupted - could not find root node")
                  );
            ...
            keyHandler: function() {
              ...
              if (this.extra_actions) {
                for (action in this.extra_actions) {
                  if (!actions[action]) {
                    actions[
                      this.extra_actions[action].binding
                    ] = this.extra_actions[action].action;
                  }
                }
              }
          ...
          Commandeger.prototype = {
            ...
            executeCommands: function(type, args) {
              ...
              for (i = 0; i < arguments.length; i += 2) {
                cmds.push(
                  makeCommand(arguments[i], arguments[i + 1], this.commands)
                );
              }
              if (this.histpos > 0) {
            ...
            doCommand: function(cmd) {
              this.working = true;
              var result = this.commands[cmd.type].apply.call(
                cmd.data,
                this.view,
                this.model
              );
            ...
            undoCommand: function(cmd) {
              this.working = true;
              this.commands[cmd.type].undo.call(
                cmd.data,
                this.view,
                this.model
              );
      ...
      5: [
        function(_dereq_, module, exports) {
          ...
          function Controller(model, o) {
            ...
            this.model.db.listen(
              "node",
              this.addFromDb.bind(this),
              this.updateFromDb.bind(this)
            );

            var actions = {};
            for (var action in this.actions) {
              if ("string" === typeof this.actions[action])
                actions[action] = this.actions[action];
          ...
          Controller.prototype = {
            ...
            actions: {
              ...
              moveInto: function(id, pid) {
                ...
                this.executeCommands("collapse", [pid, false], "move", [
                  id,
                  pid,
                  0
                ]);
              ...
              moveRight: function(id) {
                ...
                this.executeCommands("collapse", [sib, false], "move", [
                  id,
                  sib,
                  false
                ]);
              ...
              toggleCollapse: function(id, yes) {
                ...
                if (arguments.length === 1) {
                  yes =
                    !this.model.hasChildren(id) || !this.model.isCollapsed(id);
                }
                if (yes) {
                  id = this.model.findCollapser(id);
                  if (this.o.noCollapseRoot && id === this.view.root) return;
                  if (!this.model.hasChildren(id) || this.model.isCollapsed(id))
                    return;
                } else {
                  if (
                    !this.model.hasChildren(id) ||
                    !this.model.isCollapsed(id)
                  )
                    return;
                }
              ...
              remove: function(id, addText) {
                ...
                this.executeCommands("remove", [id], "appendText", [
                  before,
                  addText || ""
                ]);
              },
          ...
          DungeonsAndDragons.prototype = {
            ...
            drag: function(id, e) {
              if (this.moving.current) {
                this.vl.setDropping(
                  this.moving.current.id,
                  false,
                  this.moving.current.place === "child"
                );
              }
        ...
        function(_dereq_, module, exports) {
          ...
          DomViewLayer.prototype = {
              ...
              if (ch) {
                for (var i = 0; i < ch.length; i++) {
                  targets = targets.concat(
                    this.dropTargets(ch[i], model, moving)
                  );
                }
              }
              if (top) {
            ...
            addNew: function(node, bounds, modelActions, before, children) {
              var dom = this.makeNode(
                node.id,
                node.content,
                node.meta,
                node.depth - this.rootDepth,
                bounds,
                modelActions
              );
            ...
            showActive: function(id) {
              if (!this.dom[id])
                return console.warn(
                  "Trying to activate a node that is not rendered"
                );
            ...
            setCollapsed: function(id, isCollapsed) {
              this.dom[id].main.classList[isCollapsed ? "add" : "remove"](
                "collapsed"
              );
            },
            ...
            setMoving: function(id, isMoving) {
              ...
              this.dom[id].main.classList[isMoving ? "add" : "remove"](
                "moving"
              );
            },
        ...
        function(_dereq_, module, exports) {
          ...
          function keyHandler(keys, actions, ctrlactions) {
            ...
            for (var action in keys) {
              if (!actions[action]) {
                throw new Error(
                  "invalid configuration: trying to bind unknown action. " +
                    action
                );
              }
              bound[keys[action]] = bindAction(
                action,
                actions[action],
                ctrlactions
              );
          ...
          function bindAction(name, action, ctrlactions) {
            ...
            switch (typeof action.action) {
              case "string":
                main = ctrlactions[action.action];
                break;
              case "undefined":
                main = ctrlactions[camel(name)];
                break;
              case "function":
                main = action.action;
                break;
          ...
          function camel(string) {
            return string.replace(/ (\w)/, function(a, x) {
    ...
    */
            ...
            create: function(pid, index, text, type, meta) {
              ...
              setTimeout(
                function() {
                  ...
                  this.db.update("node", pid, {
                    children: this.ids[pid].children
                  });
                }.bind(this)
              );
            ...
            nextSibling: function(id, strict) {
              ...
              var ix = this.ids[pid].children.indexOf(id);
              if (ix < this.ids[pid].children.length - 1)
                return this.ids[pid].children[ix + 1];
            ...
            idAbove: function(id) {
              ...
              while (
                ...
                previd = this.ids[previd].children[
                  this.ids[previd].children.length - 1
                ];
              }
              return previd;
            },
      ...
      15: [
        function(_dereq_, module, exports) {
          ...
          module.exports = {
            ...
            cut: {
              help:
                "remove the currnetly selected item and place it in the clipboard",
              active: true
            },
            ...
            paste: {
              help:
                "insert the contents of the clipboard, into or below the currently selected item",
              active: true
            },

            "paste above": {
              help:
                "insert the contents of the clipboard above the currently selected item",
              active: true
            },
      ...
      23: [
        function(_dereq_, module, exports) {
          ...
          View.prototype = {
            ...
            startMoving: function(id) {
              var targets = this.vl.dropTargets(
                this.root,
                this.model,
                id,
                true
            ...
            populateChildren: function(id, node) {
              ...
              for (var i = 0; i < node.children.length; i++) {
                this.add(
                  this.model.ids[node.children[i]] || {
                    id: node.children[i],
                    parent: id,
                    content: "",
                    children: []
    ...
    */
            ...
            getChildTargets: function(cid, bid, children) {
              var targets = children
                ? children.map(this.childTarget.bind(this, bid))
                : [];
            ...
            _onMouseMoveChild: function(id, e) {
              ...
              if (this.o.startMovingChild(e, id, clone, true)) {
                this.children[id].classList.add(
     ...
     */
            ...
            createChild: function(child) {
              ...
              body.innerHTML = child.content
                ? marked(child.content)
                : "<em>Click here to edit</em>";
              var handle = document.createElement("div");
              handle.className = "handle";
              handle.innerHTML = '<i class="fa fa-circle"/>';
              handle.addEventListener(
                "mousemove",
                this._onMouseMoveChild.bind(this, child.id)
              );
              handle.addEventListener(
                "mousedown",
                this._onMouseDownChild.bind(this, child.id)
              );
            ...
            doneMovingChild: function(id) {
              this.children[id].classList.remove(
                "whiteboard-item_child--moving"
              );
            },
    ...
    */
            ...
            findTargets: function(children, id, isChild) {
              ...
              for (var i = children.length - 1; i >= 0; i--) {
                ...
                targets = targets.concat(
                  child.getChildTargets(id, children[i], childids)
                );
                targets.push(whole);
                if (!isChild) {
                  snaps.push({
              ...
              this.moving.snaps.forEach(function(snap) {
                if (!dy) {
                  ...
                  if (Math.abs(snap.top - space - b) < allowance) {
                    ...
                    y = snap.top;
                    dy = [snap.left, snap.right, snap.top - space / 2];
                  } else if (Math.abs(snap.bottom + space - y) < allowance) {
                    // bottom
                    y = snap.bottom + space;
                ...
                if (!dx) {
                  ...
                  if (Math.abs(snap.left - space - r) < allowance) {
                    ...
                    x = snap.left;
                    dx = [snap.top, snap.bottom, snap.left - space / 2];
                  } else if (Math.abs(snap.right + space - x) < allowance) {
                    // right
                    x = snap.right + space;
            ...
            _onMouseMove: function(e) {
              ...
              if (this.moving.id) {
                var box = this.container.getBoundingClientRect();
                var x =
                    e.clientX / this._zoom -
                    box.left / this._zoom -
                    this.moving.x,
                  y =
                    e.clientY / this._zoom -
                    box.top / this._zoom -
                    this.moving.y;
            ...
            stopMovingChild: function() {
              ...
              if (this.moving.currentTarget) {
                ...
                if (this.moving.currentTarget.pid == this.moving.parent_id) {
                  if (
                    pos >
                    this.model.ids[this.moving.parent_id].children.indexOf(
                      this.moving.child
                    )
                  ) {
                    pos -= 1;
                ...
                this.ctrlactions.commands(
                  "move",
                  [this.moving.child, this.moving.currentTarget.pid, pos],
                  "changeNodeAttr",
                  [this.moving.child, "whiteboard", null]
                );
              } else {
                this.ctrlactions.commands(
                  "changeNodeAttr",
                  ...
                  "move",
                  [this.moving.child, this.root, pos]
                );
              }
            ...
            stopMovingMain: function() {
              this.ids[this.moving.id].reposition(
                this.moving.atx,
                this.moving.aty
              );
              ...
              if (this.moving.currentTarget) {
                this.ctrlactions.commands(
                  ...
                  "changeNodeAttr",
                  [this.moving.id, "whiteboard", null]
                );
              }
            },
      ...
      29: [
        function(_dereq_, module, exports) {
          ...
          function WFController(model, options) {
            options.extra_commands = util.extend(
          ...
          WFController.prototype = util.extend(
      ...
      31: [
        function(_dereq_, module, exports) {
          ...
          WFModel.prototype.addTagRoot = function() {
            var index = this.ids[this.root].children
              ? this.ids[this.root].children.length
              : 0;
          ...
          WFModel.prototype.addTag = function(name) {
            ...
            var index = this.ids[tagRoot].children
              ? this.ids[tagRoot].children.length
              : 0;
          ...
          WFModel.prototype.readd = function(saved) {
            ...
            function process(node) {
              ...
              if (node.meta.tags) {
                node.meta.tags.forEach(function(id) {
                  ...
                  if (refs.indexOf(node.id) !== -1)
                    return console.warn("duplicate ref on readd");
                  refs.push(node.id);
                  upRefs[id] = true;
                });
              }
              ...
              if (node.meta.references) {
                node.meta.references.forEach(function(id) {
                  ...
                  if (tags.indexOf(node.id) !== -1)
                    return console.warn("duplicate tag on readd");
                  tags.push(node.id);
                  upTags[id] = true;
                });
              }
            ...
            for (id in upRefs) {
              this.db.update("node", id, {
                references: this.ids[id].references
              });
            }
            ...
            setTimeout(
              function() {
                ...
                for (id in upRefs) {
                  if (this.ids[id]) {
                    this.db.update("node", id, {
                      references: this.ids[id].meta.references
                    });
                  }
                }
              }.bind(this)
            ...
            while (frontier.length) {
              ...
              for (var i = 0; i < frontier.length; i++) {
                ...
                if (content && content.toLowerCase().indexOf(text) !== -1) {
                  items.push({
                    id: frontier[i],
                    text: this.ids[frontier[i]].content
                  });
                }
      ...
      32: [
        function(_dereq_, module, exports) {
          ...
          function WFNode(content, meta, actions, isNew, modelActions) {
            ...
            this.done = meta.done;
            this.tags = new Tags(
              modelActions.resolveTags(meta.tags),
              actions,
              modelActions
            );
        ...
        function(_dereq_, module, exports) {
          ...
          Tags.prototype = {
            setupNode: function() {
              ...
              this.tags.addEventListener("mousedown", function(e) {
                e.preventDefault();
              });
              this.resultsNode.addEventListener("mousedown", function(e) {
                e.preventDefault();
              });

              this.newNode = document.createElement("div");
              this.newNode.className = "treed_tags_new";
              this.newNode.addEventListener("mousedown", function(e) {
                e.preventDefault();
              });
            ...
            filterBy: function(needle) {
              ...
              if (!needle) {
                ...
                this.results = this.fullResults.filter(function(tag) {
                  return (
                    !used[tag.id] &&
                    tag.content.toLowerCase().indexOf(needle) !== -1
                  );
                });
              }
            },
            ...
            showResults: function() {
              ...
              for (var i = 0; i < num; i++) {
                ...
                node.addEventListener(
                  "click",
                  click.bind(this, this.results[i])
                );
            ...
            add: function(tag) {
              if (this.dom[tag.id])
                return console.warn("tried to add duplicate tag");
      ...
      34: [
        function(_dereq_, module, exports) {
          ...
          WFView.prototype.initialize = function(root) {
            ...
            if (node.meta.references) {
              this.vl.setReferences(
                node.meta.references.map(
                  function(id) {
                    return this.model.ids[id];
                  }.bind(this)
                ),
                this.rebase.bind(this)
              );
            }
            return rootNode;
          };
          ...
          WFView.prototype.addTree = function(node, before) {
            ...
            if (node.meta.tags) {
              node.meta.tags.forEach(
                function(id) {
                  if (id === this.root) {
                    this.vl.addReference(
                      this.model.ids[node.id],
                      this.rebase.bind(this, node.id)
                    );
                  }
                }.bind(this)
              );
          ...
          WFView.prototype.setTags = function(id, tags, oldTags) {
            ...
            for (var i = 0; i < tags.length; i++) {
              this.setAttr(
                tags[i],
                "references",
                this.model.ids[tags[i]].meta.references,
                true
              );
            }
            if (oldTags) {
              for (var i = 0; i < oldTags.length; i++) {
                ...
                this.setAttr(
                  oldTags[i],
                  "references",
                  this.model.ids[oldTags[i]].meta.references,
                  true
                );
              }
            }
      ...
      35: [
        function(_dereq_, module, exports) {
          ...
          WFVL.prototype.makeHead = function(body, actions) {
            var head = DomViewLayer.prototype.makeHead.call(
                this,
                body,
                actions
              ),
          ...
          WFVL.prototype.makeRoot = function(node, bounds, modelActions) {
            var root = DomViewLayer.prototype.makeRoot.call(
              this,
              node,
              bounds,
              modelActions
            ...
            refContainer.innerHTML =
              '<h1 class="treed_references_title">References</h1>';
